  ...
  return (
    <>
      ...
      <main className="max-w-6xl mx-auto mt-16 px-6">
        ...
        {/* Hero Image with Modal Trigger */}
        {imageUrl && (
          <div className="mb-12 cursor-pointer" onClick={() => setShowGallery(true)}>
            <img
              src={imageUrl}
              alt={place.name}
              className="w-full h-80 object-cover rounded-3xl shadow-md hover:opacity-90 transition"
            />
          </div>
        )}
              ...
              <div className="overflow-x-auto whitespace-nowrap space-x-4 px-2">
                {imageUrls.map((url, index) => (
                  <img
                    ...
                    className="inline-block h-60 rounded-xl shadow-md hover:scale-105 transition"
                  />
                ))}
              </div>
            </div>
